...
import { ArrowLeft, CalendarClock, Mail, MapPin, Package, Phone, UserPlus } from "lucide-react";
import { notFound } from "next/navigation";

import { getQuoteDetail, updateQuoteFromForm } from "@/actions/quotes";
import { convertQuoteRequestToCrmLeadAction } from "@/actions/crm-leads";
import { quoteStatusValues } from "@/lib/quotes/core";
import { Button, Input } from "@/components/ui/core";
import { StatusBadge } from "@/components/portal/status-badge";
import { getPortalUser } from "@/lib/portal-auth";
import { canConvertQuoteRequestToCrmLead, canManageQuotes } from "@/lib/portal-roles";
import { formatWibDate, formatWibDateTime, toWibDateTimeLocalValue } from "@/lib/time/wib";

type QuoteDetailProps = { params: Promise<{ id: string }>; searchParams?: Promise<{ error?: string; notice?: string }> };
...
export default async function QuoteDetailPage({ params, searchParams }: QuoteDetailProps) {
  ...
  return (
    <div className="space-y-6">
      {query?.error ? <div className="rounded-lg border border-rose-500/20 bg-rose-500/10 px-4 py-3 text-sm text-rose-200" role="alert">{query.error}</div> : null}
