const { getDB, response, errorResponse, optionsResponse, verifyToken, getAuthToken, generateTrackingNumber, sendEmail } = require('../lib/db');
...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'GET' && action === 'track') {
    ...
    try {
      ...
      return response({ shipment: shipment[0], events });
