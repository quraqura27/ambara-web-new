...
import { invoiceEffectiveStatus, isInvoiceEffectiveStatus } from "./core.ts";
import {
...
function statusCondition(status: InvoiceExportStatus) {
  ...
  if (status === "overdue") {
    return sql`${invoices.status} = 'sent' and ${invoices.paidAt} is null and ${invoices.dueDate} < current_date`;
  }
...
function exportWhere(filters: InvoiceExportFilters) {
  ...
  if (filters.payment === "paid") conditions.push(sql`${invoices.paidAt} is not null or ${invoices.status} = 'paid'`);
  if (filters.payment === "unpaid") conditions.push(sql`${invoices.paidAt} is null and ${invoices.status} <> 'paid'`);
